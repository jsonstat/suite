...
export default function datalist(jsonstat, options) {
	...
	let
		...
		dsid = options.dsid || 0,
		vlabel = options.vlabel || null, //take default value from Transform
		slabel = options.slabel || null, //take default value from Transform
		...
		tcols = function (r, i) {
			...
			r.forEach(function (e, c) {
				const cls = (colvalue === c) ? ' class="' + numclass + " " + valclass + '"' : '';
				let val = na;
	...
	if (metric) {
		metric[0].Category().forEach(function (e) {
			...
			const decimals = e.unit && e.unit.hasOwnProperty("decimals") ? e.unit.decimals : null;
			decs[e.label] = decimals;
		});
	}
	console.log(ds);
	console.log(ds.Transform({
		status: shstatus,
		vlabel: vlabel,
		slabel: slabel
	}));
